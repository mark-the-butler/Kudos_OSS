...
class Service(object):
  ...
  def find_all_kudos(self):
    kudos = self.repo_client.find_all_kudos({'user_id': self.user_id})
  ...
  def update_kudo_with(self, repo_id, githubRepo):
    records_affected = self.repo_client.update({'user_id': user_id, 'repo_id': repo_id}, self.prepare_kudo(githubRepo))
    return records_affected > 0

  def delete_kudo_for(self, repo_id):
    records_affected = self.repo_client.delete({'user_id': user_id, 'repo_id': repo_id})
    return records_affected > 0
